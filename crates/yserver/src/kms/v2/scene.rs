            ...
            drawable_snap
        {
                ...
                && intersects
            {
                ...
                draws.push(CompositeDraw {
                    ...
                    src_origin: [0.0, 0.0],
                    src_size: [1.0, 1.0],
                    // Depth-32 ARGB windows carry per-pixel alpha that
                    // must alpha-blend onto layers below — matching
                    // X11 / Composite semantics where the root window
                    // is the opaque bottom layer and ARGB windows stack
                    // with blending. Forcing alpha to 1.0 here (the old
                    // `false` setting) turned transparent areas into
                    // opaque black, hiding mate-panel applets,
                    // control-center sidebar text, system-tray icons,
                    // and tooltips. Depth-24 sources stay correct
                    // because their BgraNoAlpha image-view swizzle
                    // pins alpha=ONE at view-bind time, so this
                    // pipeline sees alpha=1 from them regardless.
                    // (Stage 4d.8a flipped the default init colour to
                    // opaque-black at every depth, but pixels that the
                    // client subsequently writes with ARGB content
                    // still need this passthrough to blend correctly.)
                    alpha_passthrough: true,
                });
                sampled_ids.push(source_id);
                if let Some(snap) = store.peek_presentation_damage(source_id) {
