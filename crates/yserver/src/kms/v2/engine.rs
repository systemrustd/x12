...
impl RenderEngine {
    ...
    /// Used by the wait-before-flush regression test to assert
    /// that `wait_for_drawable_idle` drains pending work before
    /// blocking on `ticket.wait` — without that drain, the wait
    /// blocks on a ticket whose CB is still sitting in the batch
    /// and 5-second times out, dead-locking PRESENT.
