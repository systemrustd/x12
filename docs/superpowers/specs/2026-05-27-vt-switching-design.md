...
Every wlroots+Vulkan compositor (sway, Hyprland, smithay derivatives) in production demonstrates this pattern working across logind / seatd / direct backends — because none of them need to actively keep Vulkan alive; the kernel's render-node design makes it automatic.

**Two mechanisms, same outcome.** The render-node argument above is the *proof* for Mesa-backed drivers (amdgpu/RADV, msm/Turnip, apple_drm/Asahi, nouveau, i915). For **proprietary NVIDIA**, the rendering stack doesn't use a DRM render node — nvidia's Vulkan ICD opens its own `/dev/nvidia0` / `/dev/nvidiactl` char devices, completely independent of `/dev/dri/cardN`. When we `drmDropMaster` on cardN to release the VT, nvidia's Vulkan path is untouched for the same end-result reason: it never went through our DRM fd in the first place. So we *expect* VT switching to work on proprietary NVIDIA, but the proof is by independence-of-mechanism rather than by the render-node argument, and it needs hardware validation rather than spec-level certainty. Add a proprietary-NVIDIA host to the manual hardware capture list once one is available.

For the render-node-less / primary-node-only Mesa case (rare, mostly old hardware that pre-dates render-node introduction), Risks #9 covers the failure mode and mitigation.
...
9. **Hardware / driver without a DRM render node forces Mesa onto the primary node.** On Mesa-backed KMS drivers in the tested matrix (amdgpu, msm, apple_drm/asahi, nouveau, i915 where present) Mesa renders via `/dev/dri/renderD12N`, which doesn't participate in master — see "Device fd lifetime contract". Some old or niche drivers don't expose a render node; Mesa falls back to the primary node and *does* depend on master. On those, our `drmDropMaster` invalidates Mesa's context and we'd see `VK_ERROR_DEVICE_LOST` on the first post-resume submit. Mitigation: if telemetry shows this on a tested host (none expected in the matrix), we either exit-and-restart on resume or implement full Vulkan device-loss recovery as the follow-up. Proprietary NVIDIA is expected to work via the independent-`/dev/nvidia*`-nodes mechanism (see "Device fd lifetime contract") but is unproven without hardware validation; same exit-and-restart fallback applies if it surfaces.
