...
- **Gradient-create protocol barrier** (T3, `f24eb72`): the three `RenderCreate*Gradient` entry points call `flush_if_needed(ProtocolBarrier)` before `gradient::build`. Gradients are created per-`RenderCreate*Gradient` (not per-frame), so the flush is cheap and rare. The `run_legacy_paint_op` audit catalogue (`backend.rs:~1697`) was updated to mark T1/T2 migrated and note the 3D-deferred recorders.
